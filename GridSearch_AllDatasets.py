...
l_InitialOmega = [250, 500, 750, 1000, 2500, 5000, 10000]
l_mu = [0.25, 0.5, 0.6, 0.7, 0.8, 0.9]
...
result = pd.DataFrame(columns = ['InitialOmega', 'mu', 'RMSE', 'NDEI', 'MAE', 'Rules'])
for InitialOmega in l_InitialOmega:
    for mu_threshold in l_mu:
        
        # Initialize the model
        model = exTS(InitialOmega = InitialOmega, mu_threshold = mu_threshold)
        # Train the model
        ...
        result = result.append({'InitialOmega': InitialOmega, 'mu': mu_threshold, 'RMSE': RMSE, 'NDEI': NDEI, 'MAE': MAE, 'Rules': Rules}, ignore_index=True)
