...
model = exTS(InitialOmega = InitialOmega, mu_threshold = mu_threshold, epsilon = epsilon, rho = rho)
# Train the model
...
# Plot the graphic of the actual time series and the exTS+ predictions
plt.plot(y_test1, label='Actual Value', color='red')
plt.plot(OutputTestDenormalized1, color='blue', label='exTS')
plt.ylabel('Output')
plt.xlabel('Samples')
plt.legend()
plt.show()
...
plt.plot(OutputTestDenormalized2, color='blue', label='exTS')
plt.ylabel('Output')
plt.xlabel('Samples')
plt.legend()
plt.show()
